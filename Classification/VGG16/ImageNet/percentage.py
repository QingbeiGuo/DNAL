...
from models.vgg.vgg_AutoML_ import VGG_AutoML_, vgg16_bn_AutoML_
#dataset
...
#	print("model_training:", model)
	...
	model_p = vgg16_bn_AutoML()
	model_p = torch.load("model_training").module
	print("vgg16_bn_AutoML:", model_p)
	model = vgg16_bn_AutoML_()
	print("vgg16_bn_AutoML_:", model)
	...
	for index, module in enumerate(model.modules()):
	    if module.__str__().startswith('ScaleLayer2d'):
	        ...
	        #print("index,mask", index, module.mask)

	        if module.__str__().startswith('ScaleLayer2d'):
	            model.set_conv_mask(index-2, np.where(np.array(layer_combine_mask) == 0))

...
class FineTuner_CNN:
	def __init__(self, train_path, test_path, model):
	    ...
	    self.delta_arch = self.args.delta_arch
...
##############################################################################################################
	def autoML(self):
		for i in list(range(self.stages)):
		    ...
		    self.model = torch.load("model_training_param79").cuda()
...
###############################################################
		    ...
		    self.accuracys1 = torch.load("accuracys1_trainning")[:(self.epochs_param[i]+1)]
		    self.accuracys5 = torch.load("accuracys5_trainning")[:(self.epochs_param[i]+1)]
		    self.model = torch.load("model_training_param99").cuda()
		    if self.parallel == 1:
		        self.model = torch.nn.DataParallel(self.model).cuda()
		    print("model_training:", self.model)
		    print("self.accuracys1", self.accuracys1)
		    print("self.accuracys5", self.accuracys5)

		    # fix parametersm, training scale layer
		    for module in self.model.modules():
...
###############################################################
		...
		self.accuracys1 = torch.load("accuracys1_trainning")[:(self.epochs_param[0]+self.epochs_arch[0]+2)]
		self.accuracys5 = torch.load("accuracys5_trainning")[:(self.epochs_param[0]+self.epochs_arch[0]+2)]
		self.model = torch.load("model_training_arch9").cuda()
		if self.parallel == 1:
		    self.model = torch.nn.DataParallel(self.model).cuda()
		print("model_training:", self.model)
		print("self.accuracys1", self.accuracys1)
...
###########################################################
		...
		self.accuracys1 = torch.load("accuracys1_trainning")[:(self.epochs_param[0]+self.epochs_arch[0]+3+0)]
		self.accuracys5 = torch.load("accuracys5_trainning")[:(self.epochs_param[0]+self.epochs_arch[0]+3+0)]
...
##############################################################################################################
	...
	def adjust_learning_rate(self, optimizer, epoch, iteration, num_iter):
		...
		if self.args.learning_rate_decay == 0:
		    ...
		    current_iter = iteration + (epoch + 40) * num_iter
		    max_iter = (args.epochs + 40) * num_iter
	...
	def adjust_learning_rate_param(self, optimizer, epoch, iteration, num_iter):
		...
		if self.args.learning_rate_decay == 0:
		    ...
		    max_iter = (args.epochs_param + 50) * num_iter
...
def get_args():
    ...
    parser.add_argument('--epochs', type=int, default=110, help='number of total epochs to run')
    parser.add_argument('--learning_rate', '--lr', type=float, default=0.05, help = 'initial learning rate')
    ...
    parser.add_argument('--lambda_arch', '--lambda_arch', type=float, default=5e-5, help = 'lambda')
    parser.add_argument('--threshold_arch', '--threshold_arch', type=float, default=0.9, help='threshold (default: 0.1)')
    parser.add_argument('--delta_arch', '--delta_arch', type=float, default=[30], help='delta (default: 1)')
    parser.add_argument('--delta_arch_D', '--delta_arch_D', type=float, default=1, help='delta (default: 1)')

    parser.add_argument('--epochs_param', type=int, default=[100], help='number of total epochs to optimize parameter')
    parser.add_argument('--learning_rate_param', '--lr_param', type=float, default=0.05, help = 'initial learning rate')

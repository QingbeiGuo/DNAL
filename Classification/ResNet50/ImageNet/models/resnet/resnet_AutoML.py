...
from models.resnet.layers import ScaleLayer2d, ScaleLayer1d

__all__ = ['ResNet_AutoML', 'resnet18_AutoML', 'resnet34_AutoML', 'resnet50_AutoML', 'resnet101_AutoML',
           'resnet152_AutoML']
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.bn2 = nn.BatchNorm2d(planes)
        self.scale2 = ScaleLayer2d(planes)
        self.conv3 = MaskedConv2d(planes, planes * Bottleneck.expansion, kernel_size=1, bias=False)
...
def resnet18_AutoML(pretrained=False, model_root=None, **kwargs):
    model = ResNet_AutoML(BasicBlock, [2, 2, 2, 2], **kwargs).cuda()
    if pretrained:
        misc.load_state_dict(model, model_urls['resnet18'], model_root)
    return model


def resnet34_AutoML(pretrained=False, model_root=None, **kwargs):
    model = ResNet_AutoML(BasicBlock, [3, 4, 6, 3], **kwargs).cuda()
    if pretrained:
        misc.load_state_dict(model, model_urls['resnet34'], model_root)
    return model


def resnet50_AutoML(pretrained=False, model_root=None, **kwargs):
    model = ResNet_AutoML(Bottleneck, [3, 4, 6, 3], **kwargs).cuda()
    if pretrained:
        misc.load_state_dict(model, model_urls['resnet50'], model_root)
    return model


def resnet101_AutoML(pretrained=False, model_root=None, **kwargs):
    model = ResNet_AutoML(Bottleneck, [3, 4, 23, 3], **kwargs).cuda()
    if pretrained:
        misc.load_state_dict(model, model_urls['resnet101'], model_root)
    return model


def resnet152_AutoML(pretrained=False, model_root=None, **kwargs):
    model = ResNet_AutoML(Bottleneck, [3, 8, 36, 3], **kwargs).cuda()
    if pretrained:
